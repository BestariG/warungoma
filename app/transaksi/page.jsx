...
const TransactionPage = () => {
  ...
  useEffect(() => {
    ...
    setCustomer(prev => ({ ...prev, table: generateTableNumber() }));
  }, []);
  ...
  const decreaseQty = (name) => {
    ...
    if (!updatedCart[name]) return;
  
    const product = menuItems.find(item => item.name === name);
  
    if (updatedCart[name].qty > 1) {
      updatedCart[name] = {
        ...updatedCart[name],
        qty: updatedCart[name].qty - 1,
        product_id: updatedCart[name].product_id || product?.id || null,
      };
    } else {
  ...
  const validateOrder = () => {
    const newErrors = {};
    if (!customer.name.trim()) newErrors.name = "Nama tidak boleh kosong";
    if (!/^08[0-9]{8,11}$/.test(customer.phone)) newErrors.phone = "Nomor HP tidak valid (harus mulai dari 08 dan 10–13 digit)";
    if (!customer.table.trim()) newErrors.table = "Nomor meja harus diisi";
    if (Object.keys(cart).length === 0) newErrors.cart = "Keranjang belanja kosong";
  ...
  const handleReviewOrder = () => {
    ...
    console.log("🛒 Final cart before order:", cart); // <--- Tambahkan ini
  ...
  return (
    <div className="bg-[#E6C48E] min-h-screen flex justify-center items-center p-6">
      <div className="bg-[#F5E1B7] p-6 rounded-xl shadow-lg max-w-md w-full">
        ...
        {errors.cart && <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-2 rounded mb-4">{errors.cart}</div>}
        ...
        <button className="w-full bg-[#5E031C] text-white py-2 mt-4 rounded-lg" onClick={handleReviewOrder}>
          Lanjut ke Konfirmasi
        </button>

        <button className="w-full bg-[#5E031C] text-white py-2 mt-4 rounded-lg" onClick={() => router.push('/menu')}>
          Kembali
        </button>
      </div>
    </div>
